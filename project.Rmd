...
```{r}
# Arianna's code 11/29/21 below: 

# get state and population info from wikipedia. Turn into a frame called state_pop
...
# get cumulative administrated doses      
cdc.df.50.total <- cdc.df.50 %>%
                  group_by(location) %>%
                  arrange(location) %>%
                  filter( date <= "2021-06-30",date > "2021-03-1") %>%
                  summarize(total_admin = sum(administered))


#combine both frames
cdc.df.50.final <- cdc.df.50.total %>% 
                    mutate(state_pop) %>%
                    select(-state)
