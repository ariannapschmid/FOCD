...
cov <- percent.rep %>% full_join(med.income, by = "location") %>% full_join(percent.age, by = "location")
...
```{r}
# Joining the gtrends dataset with vaccine info dataset

Sept21.analysis <- vax.Sept21 %>% 
      select(location, date, admin_per_100k, series_complete_pop_pct) %>% 
      full_join(hits.results.jan, by = "location") %>%
      arrange(location)
```
